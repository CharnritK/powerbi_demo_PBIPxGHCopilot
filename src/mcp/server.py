"""MCP server discovery and process client."""
...
from src.mcp.safety import server_executable_name
...
def discover_server_binary() -> Path:
    ...
    if not candidates:
        raise MCPError(
            "Power BI Modeling MCP server binary was not found under ~/.vscode/extensions. Install the 'analysis-services.powerbi-modeling-mcp' VS Code extension first."
        )
...
class PowerBIModelingMCPClient:
    ...
    def initialize(self) -> None:
        self.request(
            "initialize",
            {
                ...
                "capabilities": {},
                "clientInfo": {"name": "powerbi-demo-script", "version": "2.0"},
            },
        )
    ...
    def _raise_if_exited(self) -> None:
        ...
        if self._process.poll() is not None:
            raise MCPError(f"MCP process exited with code {self._process.returncode}.\n{self.stderr_tail()}")
